...
\begin{center}
\begin{Large}
05.02.17. Занятие 9. \\ Вещественный бин.поиск. Бин.поиск по ответу.\\
\end{Large}
...
Заметим, что в строке 15 сравнение вещественных чисел производется без eps. Почему так? Допустим, мы будем сравнивать числа $m*m$ и $y$ с точностью eps. Но $y = \sqrt{x}$, поэтому точность для $x$ уже будет составлять $\sqrt{eps}$. При $eps = 10^{-6}$, это всего $10^{-3}$, что не очень хорошо и может привести к WA.
